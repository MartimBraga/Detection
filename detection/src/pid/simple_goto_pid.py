...
def send_ned_velocity(velocity_x, velocity_y, velocity_z, duration):
    ...
    for x in np.arange(0,duration,0.001):
        vehicle.send_mavlink(msg)
        time.sleep(0.001)
...
class Position():
    ...
    def pose(self):
            ...
            while pidX.SetPoint is not None:
                ...
                if self.outputX > 10:
                    self.outputX = 10
                elif self.outputX < -10:
                    self.outputX = -10
                #print (self.outputX)
                
                if self.outputY > 10:
                    self.outputY = 10
                elif self.outputY < -10:
                    self.outputY = -10
                #print (self.outputY)
                ...
                send_ned_velocity(self.outputX, self.outputY, -self.outputZ, 0.025)
                end_time = time.time()
                ...
                print("-------------------------------------")
                print ("velocity X- ", str(outputstartX), "Y- ", str(outputstartY), "Y- ", str(outputstartY))
                ...
                if (abs(totalX)<= 0.10 and abs(totalY)<= 0.10 and abs(totalZ)<= 0.3):
                    break
        
                #export to xls
                wb = xlwt.Workbook()
                ws = wb.add_sheet("data")
                for i, row in enumerate(self.setpoint_listX):
                    ws.write(i, 0, row)
                for i, row2 in enumerate(self.setpoint_listY):
                    ws.write(i, 1, row2)
                for i, row3 in enumerate(self.setpoint_listZ):
                    ws.write(i, 2, row3)
                ...
                for i, row11 in enumerate(time2X):
                    ws.write(i, 10, row11)
                ...
                for i, row28 in enumerate(dronex):
                    ws.write(i, 27,row28)
                for i, row29 in enumerate(droney):
                    ws.write(i, 28,row29)
                for i, row30 in enumerate(dronez):
                    ws.write(i, 29,row30)
...
def precise_land(height):
    """ listener(1)
    #pidcontrol.pose()
    destination = RS_pose
    destination.x = vehicle.location.local_frame.north - RS_pose.x
    destination.y = vehicle.location.local_frame.east + RS_pose.y
    destination.z = vehicle.location.local_frame.down + RS_pose.z - height  """
    listener(1)
...
while landed == 0:
    ...
    if RS_pose.detection==1:
        ...
        destination.x = vehicle.location.local_frame.north - RS_pose.x
        destination.y = vehicle.location.local_frame.east + RS_pose.y
        destination.z = vehicle.location.local_frame.down + RS_pose.z -1 
        goto_position_target_local_ned(-RS_pose.x, RS_pose.y, vehicle.location.local_frame.down)
        while (abs(vehicle.location.local_frame.north - destination.x) >= 0.1 or 
                abs(vehicle.location.local_frame.east - destination.y) >= 0.1 or
                abs(vehicle.location.local_frame.down - destination.z) >= 0.1):
            time.sleep(1) """
    else:
        continue  
